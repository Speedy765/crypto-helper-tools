cryptotracky.controller('overviewLongKucoinController', function($rootScope, $http, $scope, localStorageService, ChartService) {

	$scope.activeState = "kucoin";
	...
	function updateData(keepOldData){
		if(tempResponse == [] || !keepOldData){
			backend = "https://overview-long.cryptotracky.com/min-kucoin?intervals=" + $scope.inputIntervals;
			// backend = "http://localhost:1338/min-kucoin?intervals=" + $scope.inputIntervals;
			$http.get(backend).
			then(handleResponse);
				...
				if (ignoreListLong.indexOf(key) == -1){
					...
					if(coinData.volume.toFixed(0) > parseInt(overviewSettings.minVolume)){
						$rootScope.finalList.push({
							...
							exchangeLink: "https://www.kucoin.com/#/trade.pro/" + coinData.coin + "-BTC",
