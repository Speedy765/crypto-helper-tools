cryptotracky.controller('overviewLongHuobiController', function($rootScope, $http, $scope, localStorageService, ChartService) {

	$scope.activeState = "huobi";
	...
	function updateData(keepOldData){
		if(tempResponse == [] || !keepOldData){
			backend = "https://overview-long.cryptotracky.com/min-huobi?intervals=" + $scope.inputIntervals;
			// backend = "http://localhost:1336/min-huobi?intervals=" + $scope.inputIntervals;
			$http.get(backend).
			then(handleResponse);
