cryptotracky.controller('overviewLongCryptopiaController', function($rootScope, $http, $scope, localStorageService, ChartService) {
	...
	function updateData(keepOldData){
		if(tempResponse == [] || !keepOldData){
			backend = "https://overview-long.cryptotracky.com/min-cryptopia?intervals=" + $scope.inputIntervals;
			// backend = "http://34.240.107.131:1336/min-cryptopia?intervals=" + $scope.inputIntervals;
			$http.get(backend).
			then(handleResponse);
